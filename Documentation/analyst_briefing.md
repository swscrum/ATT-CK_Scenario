...
| **T1021.004** SSH Lateral | `logs/workstation/auth.log` + `logs/router/ulog-iptables.log` | `Accepted publickey for john.stravidis from 10.30.0.4` in workstation auth.log (10.30.0.4 = router's internal-net leg post-NAT — apache's traffic is masqueraded as it transits the router). At L3 router NFLOG shows the pre-NAT source: `FW-NEW: SRC=10.40.0.2 DST=10.30.0.5 DPT=22` |
