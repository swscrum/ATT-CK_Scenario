...
- Uses a Linux dev/staging workstation that Waystar's transition team set up for him. Throwaway credentials (`labuser` / `labpass`). After handover, the box is never decommissioned — the transition team's queue is too long and the workstation doesn't appear on any decommissioning list because it was set up off-process.
